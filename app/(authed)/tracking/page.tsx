...
export default function AiTrackingPage() {
  ...
  // use the tracking schema explicitly
  const tracking = supabase.schema('tracking');

  /* --------------------------------------------------
     Load games for this week (via draft board RPC)
  -------------------------------------------------- */
  ...
  const scorePicks = async () => {
    type ScoreGame = { id: number; home_score: number | null; away_score: number | null };
    ...
    if (error) {
      console.error('scorePicks load games error', error);
      return;
    }

    const gameMap = new Map<number, ScoreGame>();
    (data ?? []).forEach((g) => gameMap.set((g as ScoreGame).id, g as ScoreGame));
    ...
    for (const p of picks) {
      ...
      const diff = g.home_score - g.away_score; // home - away
      const total = g.home_score + g.away_score;

      if (p.pick_type === 'spread' && p.team_short && p.line_or_total != null) {
        const teamIsHome =
          p.home_short &&
          p.team_short.toUpperCase() === p.home_short.toUpperCase();
  ...
  const addNewPick = async () => {
    ...
    const game = games.find((g) => g.game_id === Number(newPick.game_id));

    await tracking.from('ai_recommendations').insert([
      {
        season_year: year,
        week_number: week,
        game_id: Number(newPick.game_id),
        home_short: game?.home_short ?? null,
        away_short: game?.away_short ?? null,
        pick_type: newPick.pick_type, // must be 'spread' or 'ou'
        team_short: newPick.pick_type === 'spread' ? newPick.team_short || null : null,
        ou_side:
            ...
            <select
              ...
              onChange={(e) =>
                setNewPick((prev) => ({
                  ...
                  team_short: e.target.value === 'spread' ? prev.team_short : '',
                  ou_side: e.target.value === 'ou' ? prev.ou_side : '',
                }))
              }
            >
