...
export default function AdminScoresPage() {
  ...
  async function saveScore(row: GameForScore) {
    // basic validation
    const hs = row.home_score ?? 0;
    const as = row.away_score ?? 0;
    setSavingId(row.game_id);
    const { error, data } = await supabase.rpc('set_final_score', {
      p_year: YEAR,
      p_week_number: week,
      p_home_short: row.home,
      p_away_short: row.away,
      p_home_score: hs,
      p_away_score: as
    });
    setSavingId(null);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <p className="text-xs text-gray-400">
        Tip: after saving all games for a week, reload the main page to see results colours and the summary update.
      </p>
    </div>
  );
}
